...
from pynumaflow.batchmapper import Messages, Message, DROP, BatchResponses
...
class TestBatchResponses(unittest.TestCase):
    @staticmethod
    def mock_message_object(value):
        return Message(value=value)

    def test_batch_responses_create(self):
        mock_msgs = [
            self.mock_message_object(b"asdf"),
            self.mock_message_object(b"jkl;"),
        ]
        msgs = Messages(*mock_msgs)
        response = BatchResponses("abc", msgs)
        assert response.id == "abc"
        assert len(response.messages) == 2
        for i in range(len(response.messages)):
            self.assertEqual(type(mock_msgs[i]), type(response.messages[i]))
            self.assertEqual(mock_msgs[i].keys, response.messages[i].keys)
            self.assertEqual(mock_msgs[i].value, response.messages[i].value)

    def test_batch_responses_empty(self):
        response = BatchResponses("abc", Messages())
        assert response.id == "abc"
        assert len(response.messages) == 0

    def test_batch_responses_easy_drop(self):
        drop_response = BatchResponses.to_drop("abc")
        assert len(drop_response.messages) == 1
        assert DROP in drop_response.messages[0].tags
